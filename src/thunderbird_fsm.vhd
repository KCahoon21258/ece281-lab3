...
--| AUTHOR(S)     : C3C Kimberly Cahoon, Capt Phillip Warner, Capt Dan Johnson
...
o_lights_L(2) <= (not f_Q(2) AND not f_Q(1) AND f_Q(0)) OR (f_Q(2) AND not f_Q(1) AND f_Q(0)) OR (f_Q(2) AND f_Q(1) AND not f_Q(0)) OR (f_Q(2) AND f_Q(1) AND f_Q(0));
...
o_lights_R(2) <= (not f_Q(2) AND not f_Q(1) AND f_Q(0)) OR (not f_Q(2) AND f_Q(1) AND not f_Q(0)) OR (not f_Q(2) AND f_Q(1) AND f_Q(0))OR ( f_Q(2) AND not f_Q(1) AND not f_Q(0));
    ...
    begin
        --register state is all lights offs
        if i_reset = '1' then 
            f_Q <= "000";
